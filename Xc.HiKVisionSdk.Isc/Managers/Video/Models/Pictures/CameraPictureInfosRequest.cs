...
namespace Xc.HiKVisionSdk.Isc.Managers.Video.Models.Pictures
{
    ...
    public class CameraPictureInfosRequest : IOSTimeRangeWithPagedRequest
    {
        ...
        /// </summary>
        public string NetProtocol { get; set; } = "https";
        /// <summary>
        /// 图片排序方式。0-按监控点排序，根据监控点名称ASCII码顺序排序。同一监控点的图片按照时间倒序排序；1-按时间排序，根据抓图时间倒序排序，最新的图片排在最前边。相同时间按监控点名称顺序排序。参数不填，默认按0-监控点排序。
        /// </summary>
        public CameraPictureInfoSortType Sort { get; set; }
        /// <summary>
        /// 图片质量压缩比。标识返回缩略图的图片质量压缩比。通过修改图片压缩比来改变图片大小（图片占用字节数），不会改变图片尺寸。取值范围为0-99，不传时默认为50，取值越小，表示压缩更多，图片失真越严重。
        /// </summary>
        public int CompressRatio { get; set; } = 50;
        ...
        public CameraPictureInfosRequest(int pageNo, int pageSize, DateTime start, DateTime end) : base(pageNo, pageSize, start, end, true) { }

        /// <summary>
        /// 使用http
        /// </summary>
        /// <returns></returns>
        public CameraPictureInfosRequest UseHttp()
        {
            NetProtocol = "http";
            return this;
        }
