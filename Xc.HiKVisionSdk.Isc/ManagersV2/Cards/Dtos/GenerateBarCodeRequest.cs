...
namespace Xc.HiKVisionSdk.Isc.ManagersV2.Cards.Dtos
{
    ...
    public class GenerateBarCodeRequest : BaseRequest
    {
        ...
        protected GenerateBarCodeRequest(string personId, string cardNo, CertificateType? certType, string certNum, string phone, string employeeId, int duration = 1440, int maxLock = 4)
        {
            PersonId = personId;
            CardNo = cardNo;
            CertType = certType;
        ...
        /// <param name="duration">人员卡片二维码有效期时长，非必填字段，单位为分钟，范围为[5~7*24*60]分钟，不传默认为24*60分钟</param>
        /// <param name="maxLock">最大开锁次数，其中针对同一台设备最大开锁次数不超过指定的次数，不传默认最大开锁次数为4次</param>
        public static GenerateBarCodeRequest NewByCert(CertificateType certType, string certNum, int duration = 1440, int maxLock = 4)
        {
